...
import { loginUser, setSessionCookie, clearSessionCookie, requireSession, requireDirector, sanitizeUser, getSessionUser, revokeSessions, changeOwnPassword } from './auth.js';
import { listAccounts, saveAccount, removeAccount } from './accounts.js';
import { ensureBackendReady } from './backend-ready.js';
...
router.use((req,res,next)=>{
  ...
  if(origin){
    ...
    try{
      if(!hosts.includes(new URL(origin).host)&&origin!==process.env.APP_ORIGIN)return res.status(403).json({error:'Nguồn yêu cầu không hợp lệ.'});
...
router.post(['/api/auth/temporary','/api/auth/restore'],(req,res)=>res.status(410).json({error:'Vui lòng đăng nhập bằng tài khoản và mật khẩu.',code:'LOGIN_REQUIRED'}));
...
router.post('/api/auth/login',async(req,res,next)=>{
  ...
  try{await ensureBackendReady();next();}catch(err){next(err);}
  ...
  try{
    ...
    res.json({ok:true,user:sanitizeUser(user),firebaseToken:null});
  }catch(err){next(err);}
});

router.get('/api/auth/me',requireSession,(req,res)=>res.json({ok:true,user:sanitizeUser(req.user),firebaseToken:null}));
...
router.get('/api/admin/users',requireDirector,async(req,res,next)=>{try{res.json({users:await listAccounts(req.user)});}catch(err){next(err);}});
router.post('/api/admin/users',requireDirector,async(req,res,next)=>{try{const user=await saveAccount(req.user,req.body);res.status(201).json({ok:true,user:sanitizeUser(user)});}catch(err){next(err);}});
...
router.delete('/api/admin/users/:id',requireDirector,async(req,res,next)=>{try{await removeAccount(req.user,req.params.id);res.json({ok:true});}catch(err){next(err);}});

router.get(['/', '/index.html', '/login', '/login.html'],(req,res)=>{
  res.setHeader('Cache-Control','no-store');res.sendFile(path.join(publicDir,'login.html'));
});
router.get(['/app','/app.html'],async(req,res,next)=>{
  try{
    const user=await getSessionUser(req);
    if(!user)return res.redirect(303,'/');
    res.setHeader('Cache-Control','no-store');res.sendFile(path.join(publicDir,'app.html'));
  }catch(err){next(err);}
});
